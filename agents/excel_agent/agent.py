...
from agents.prompt.mrkl_prompt import FORMAT_INSTRUCTIONS
...
from agents.excel_agent.prompt import (
    FUNCTIONS_WITH_DF,
...
from agents.excel_agent.tools import PythonAstREPLTool,PythonAstREPLToolx,PythonAstREPLTooly, Requirment_tool,PythonREPLTool
...
def create_pandas_dataframe_agent(
    ...
    **kwargs: Any,
    ...
    if isinstance(df, list):
        ...
        df_locals["df"] = df
    print(df_locals)
    tools = [PythonAstREPLTooly(locals=df_locals)]+[PythonAstREPLToolx(locals=df_locals)]+[Requirment_tool()]+[PythonAstREPLTool(locals=df_locals)]
    ...
    elif agent_type in (AgentType.OPENAI_FUNCTIONS, "openai-tools", "tool-calling","base_require_tool"):
